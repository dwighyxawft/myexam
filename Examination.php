...
class Examination {
    ...
    function __construct(){
        ...
        $this->dbms = "myexam";
        $this->home = "http://localhost:8080/myexam/";
    ...
    function admin_session_public(){
        if(!isset($_SESSION["admin_id"])){
          echo $this->redirect("admin/login.php");
        }
    }
    function admin_session_private(){
        if(isset($_SESSION["admin_id"])){
          echo $this->redirect("admin/home.php");
        }
    }
    function user_session_public(){
        if(!isset($_SESSION["user_id"])){
          echo $this->redirect("user/login.php");
        }
    }
    function user_session_private(){
        if(isset($_SESSION["user_id"])){
          echo $this->redirect("user/home.php");
        }
    }
