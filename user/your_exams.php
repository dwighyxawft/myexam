    ...
    <main>
        <div class="container">
            <div class="row">
                ...
                <div class="col-md-3 mt-4">
                <button class="btn w-100" id="pend_exams" style="background-color: transparent;">
                <div class="cards rounded bg-success">
                        <div class="w-50 bg-light rounded border border-1 border-success"><h5 class="text-center text-success py-2">Pending Exams</h5></div>
                        <div class="pt-5 pb-2">
                            <div class="row">
                                ...
                                <div class="col d-flex justify-content-end"> <h1 class="cards-text d-inline text-end text-light pe-4">
                                    <?php
                                    $exam->query = "SELECT * FROM user_enroll_exam_table WHERE (offerer_id = '$user_id' OR enroller_id = '$user_id') AND acceptance_status = 'pending'";
                ...
                <div class="col-md-3 mt-4">
                    <button class="btn w-100" id="accepted_exams" style="background-color: transparent;">
                    <div class="cards rounded bg-primary">
                        <div class="w-50 bg-light rounded border border-1 border-primary"><h6 class="text-center text-primary py-2">Accepted Exams</h6></div>
                        <div class="pt-5 pb-2">
                            <div class="row">
      ...
      <div class="container">
       <div class="table-responsive">
            <table class="table table-hover table-bordered border-success mt-5 mb-5">
                <thead>
                    <tr>
        ...
        <ul class="pagination pagination-md pb-5 pt-4 pend_exam">
        <?php
                                            $exam->query = "SELECT * FROM user_enroll_exam_table WHERE (offerer_id = '$user_id' OR enroller_id = '$user_id') AND acceptance_status = 'pending'";
...
<ul class="pagination pagination-md pb-5 pt-4 accepted_exam">
    ...
    <script>
        $(document).ready(function(){
            ...
            $("#pend_exams").on("click", function(){
                get_exams(0, "pending");
                $("ul").hide();
                $("ul.pend_exams").show();
            })
            $("#accepted_exams").on("click", function(){
                get_exams(0, "accepted");
                $("ul").hide();
                $("ul.accepted_exams").show();
            })
            $("ul.pend_exams button").click(function(){
                var value = $(this).text();
                var actual = value - 1;
                var search = actual * 10;
                console.log(search);
                get_exams(search, "pending");
            })
